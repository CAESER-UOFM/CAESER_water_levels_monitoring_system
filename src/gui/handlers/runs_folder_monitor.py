...
class RunsFolderMonitor:
    def __init__(self, folder_id=None, db_path=None):
        # Use consolidated folder if available, otherwise fall back to hardcoded default
        self.folder_id = folder_id or "1FhCJH6KuvHcdFSpn0PxY8k9_-62vjmKj"
        logger.info(f"RunsFolderMonitor initialized with folder_id: {self.folder_id}")
        logger.info(f"Folder ID source: {'provided' if folder_id else 'hardcoded fallback'}")
        self.service = None
    ...
    def set_authenticated_service(self, service):
        ...
        # Try to update folder ID to consolidated folder if available
        # This ensures we use the correct folder even if initialization failed
        try:
            # Try to get the main window and settings handler
            import inspect
            for frame_info in inspect.stack():
                frame = frame_info.frame
                if 'self' in frame.f_locals and hasattr(frame.f_locals['self'], 'window'):
                    main_window = frame.f_locals['self'].window()
                    if main_window and hasattr(main_window, 'settings_handler'):
                        ...
                        if consolidated_folder_id and consolidated_folder_id != self.folder_id:
                            logger.info(f"RunsFolderMonitor: Updating folder_id from {self.folder_id} to consolidated folder {consolidated_folder_id}")
                            self.folder_id = consolidated_folder_id
                        break
        except Exception as e:
            logger.debug(f"Could not update folder ID during service set: {e}")
        
        logger.info(f"RunsFolderMonitor: Authenticated service set. Folder ID: {self.folder_id}")
        return True
    ...
    def get_month_folders(self, year_month):
        ...
        if not self.service:
            logger.warning("get_month_folders called but Google Drive service not authenticated yet")
            return {}
            
        try:
            # Keep year_month format as-is (2025-02) since cloud folders use hyphens
            current_folder = year_month
            
            # Calculate next month folder name with hyphen format
            ...
            if not folders:
                logger.warning(f"No folders found matching {current_folder} or {next_month} in parent folder {self.folder_id}")
                # Try to list ALL folders in the parent to see what's available
                try:
                    all_folders_query = f"'{self.folder_id}' in parents and mimeType = 'application/vnd.google-apps.folder' and trashed = false"
                    ...
                    if all_folders:
                        ...
                        logger.warning(f"Available folders in parent {self.folder_id}: {folder_names}")
                    else:
                        logger.warning(f"No folders found at all in parent {self.folder_id}")
                except Exception as debug_e:
                    logger.error(f"Error listing all folders for debugging: {debug_e}")
    ...
    def extract_date_from_filename(self, filename):
        ...
        try:
            ...
            if '_To_' in filename:
                ...
                # Try timestamp format: _Weather Station_2025_04_30_100000_2025_05_28_101736.xle
                # Look for pattern with two dates separated by timestamps
                import re
                # Pattern: anything_YYYY_MM_DD_HHMMSS_YYYY_MM_DD_HHMMSS.xle
                pattern = r'(.+?)_(\d{4})_(\d{2})_(\d{2})_(\d{6})_(\d{4})_(\d{2})_(\d{2})_(\d{6})\.xle'
                match = re.match(pattern, filename)
                
                if match:
                    # Extract end date components
                    end_year = match.group(6)
                    end_month = match.group(7)
                    end_day = match.group(8)
                    
                    # Create date string and convert to datetime
                    date_str = f"{end_year}-{end_month}-{end_day}"
                    logger.debug(f"Extracted date from timestamp format: {date_str}")
                    
                    # Make a datetime object at the END of the day
                    dt = datetime.strptime(f"{date_str} 23:59:59", "%Y-%m-%d %H:%M:%S")
                    return dt
                else:
                    logger.debug(f"Could not parse filename format: {filename}")
                    return None
    ...
    def get_folder_for_file(self, filename):
        ...
        try:
            # P-210_2024_08_12_To_2025_02_10.xle -> 2025-02
            end_date = filename.split('_To_')[1].split('.')[0]  # Gets 2025_02_10
            date_parts = end_date.split('_')[:2]  # Gets ['2025', '02']
            folder_name = '-'.join(date_parts)    # Gets 2025-02

...
class WaterLevelEditDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        # Create main vertical splitter between controls and plot
        ...
        # Create horizontal splitter for control groups
        controls_splitter = QSplitter(Qt.Horizontal)
        controls_splitter.setChildrenCollapsible(False)
        
        # Left side controls
        left_controls = QGroupBox("Data Filters")
        left_layout = QVBoxLayout()
        
        # Add filter checkboxes
        self.show_master_baro = QCheckBox("Show Master Baro")
        self.show_baro_flag = QCheckBox("Show Baro Flags")
        self.show_level_flag = QCheckBox("Show Level Flags")
        ...
        left_layout.addWidget(self.show_master_baro)
        left_layout.addWidget(self.show_baro_flag)
        left_layout.addWidget(self.show_level_flag)
        left_controls.setLayout(left_layout)
        
        # Middle controls - Edit Tools
        edit_controls = QGroupBox("Edit Tools")
        edit_layout = QVBoxLayout()
        
        # Add edit tool buttons
        self.fix_spikes_btn = QPushButton("Fix Spikes")
        self.compensation_btn = QPushButton("Compensation")
        self.adjust_baseline_btn = QPushButton("Adjust Base Line")
        ...
        # Style the buttons to be more prominent
        ...
        # Add calculator button to the edit tools
        ...
        edit_controls.setLayout(edit_layout)
        
        # Right side controls
        right_controls = QGroupBox("Time Range")
        ...
        self.start_min_btn = QPushButton("⏮")  # Using begin symbol
        self.start_min_btn.setToolTip("Set to earliest data point")
        self.start_min_btn.setFixedSize(24, 24)
        self.start_min_btn.clicked.connect(self.set_start_to_first_point)
        start_layout.addWidget(self.start_min_btn)
        ...
        self.end_max_btn = QPushButton("⏭")  # Using end symbol
        self.end_max_btn.setToolTip("Set to latest data point")
        self.end_max_btn.setFixedSize(24, 24)
        self.end_max_btn.clicked.connect(self.set_end_to_last_point)
        end_layout.addWidget(self.end_max_btn)
        
        # Zoom button
        self.zoom_to_selection_btn = QPushButton("Zoom to Selected Data")
        ...
        right_layout.addLayout(end_layout)
        right_layout.addWidget(self.zoom_to_selection_btn)
        right_layout.addWidget(self.full_range_btn)
        right_controls.setLayout(right_layout)
        
        # Add all control panels to the horizontal splitter
        controls_splitter.addWidget(left_controls)
        controls_splitter.addWidget(edit_controls)
        controls_splitter.addWidget(right_controls)
        
        # Set reasonable initial sizes for the control panels (relative)
        controls_splitter.setSizes([200, 200, 300])
        
        # Add the controls splitter to the controls layout
        ...
        # Add controls and plot to the main splitter
        main_splitter.addWidget(controls_widget)
        main_splitter.addWidget(plot_widget)
        
        # Set initial sizes for main sections (control area should be smaller)
    ...
    def reset_baseline(self):
        """Reset baseline adjustment for the current instance"""
        try:
            # Remove preview line if it exists
            if hasattr(self, 'baseline_preview_line') and self.baseline_preview_line:
                self.baseline_preview_line.remove()
                self.baseline_preview_line = None
            
            # Reset edits for this instance
            if hasattr(self, 'baseline_helper') and self.baseline_helper:
                self.reset_instance_edits(self.baseline_helper.instance_id)
            
            logger.debug("Reset baseline adjustments")
            
        except Exception as e:
            logger.error(f"Error resetting baseline: {e}", exc_info=True)
            QMessageBox.warning(self, "Error", f"Failed to reset baseline: {str(e)}")
    ...
    def apply_instance_edits(self, instance_id, method, changes):
        ...
        try:
            ...
            for idx in affected_indices:
                original_values[idx] = {
                    'water_level': self.transducer_data.loc[idx, 'water_level'].copy() if idx in self.transducer_data.index else None,
                    'water_level_spike_corrected': self.transducer_data.loc[idx, 'water_level_spike_corrected'].copy() if idx in self.transducer_data.index else None,
                    'water_level_master_corrected': self.transducer_data.loc[idx, 'water_level_master_corrected'].copy() if idx in self.transducer_data.index else None,
                    'water_level_level_corrected': self.transducer_data.loc[idx, 'water_level_level_corrected'].copy() if idx in self.transducer_data.index else None,
                    'baro_flag_mod': self.transducer_data.loc[idx, 'baro_flag_mod'].copy() if idx in self.transducer_data.index else None,
                    'level_flag_mod': self.transducer_data.loc[idx, 'level_flag_mod'].copy() if idx in self.transducer_data.index else None,
                    'spike_flag': self.transducer_data.loc[idx, 'spike_flag'].copy() if idx in self.transducer_data.index else None,
                }
    ...
    def apply_baseline_changes(self):
        """Apply baseline changes based on the current parameters from the helper dialog"""
        try:
            # Check for selected data
            if self.selected_data is None or self.selected_data.empty:
                # Get current date range from the plot view
                xlim = self.ax.get_xlim()
                x_min = matplotlib.dates.num2date(xlim[0]).replace(tzinfo=None)
                x_max = matplotlib.dates.num2date(xlim[1]).replace(tzinfo=None)
                
                # Use visible data range if no selection is made
                visible_mask = (self.plot_data['timestamp_utc'] >= x_min) & \
                              (self.plot_data['timestamp_utc'] <= x_max)
                self.selected_data = self.plot_data[visible_mask].copy()
                
                if self.selected_data.empty:
                    # Create message box that stays on top
                    msg_box = QMessageBox(self)
                    msg_box.setWindowTitle("Error")
                    msg_box.setText("No data selected for baseline adjustment.")
                    msg_box.setIcon(QMessageBox.Warning)
                    msg_box.setWindowFlags(msg_box.windowFlags() | Qt.WindowStaysOnTopHint)
                    msg_box.exec_()
                    return
            ...
            # Get current parameters from the dialog

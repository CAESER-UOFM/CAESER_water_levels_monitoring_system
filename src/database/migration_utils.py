...
class DatabaseMigration:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                migrations = [
                    ...
                    self._migrate_temperature_spike_corrected,
    ...
    def _migrate_temperature_spike_corrected(self, cursor: sqlite3.Cursor) -> bool:
        """
        Add temperature_spike_corrected column to water_level_readings table if it doesn't exist.
        ...
        try:
            ...
            if 'temperature_spike_corrected' in columns:
                return False  # Column already exists
            
            # Add the column
            cursor.execute('''
                ALTER TABLE water_level_readings 
                ADD COLUMN temperature_spike_corrected REAL
            ''')
            
            # Set initial values to match temperature column for existing records
            cursor.execute('''
                UPDATE water_level_readings 
                SET temperature_spike_corrected = temperature 
                WHERE temperature_spike_corrected IS NULL
            ''')
            
            logger.info("Added temperature_spike_corrected column to water_level_readings table")
            return True
            
        except Exception as e:
            logger.error(f"Error adding temperature_spike_corrected column: {e}")
            raise

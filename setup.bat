...
REM Main app shortcut pointing to debug batch file (shows console for visibility)
powershell -ExecutionPolicy Bypass -Command "try { $WshShell = New-Object -comObject WScript.Shell; $Shortcut = $WshShell.CreateShortcut('%INSTALL_DIR%\CAESER Water Levels Monitoring.lnk'); $Shortcut.TargetPath = '%INSTALL_DIR%\launchers\water_levels_monitoring_system_debug.bat'; $Shortcut.WorkingDirectory = '%INSTALL_DIR%'; $Shortcut.Description = 'CAESER Water Levels Monitoring System'; if (Test-Path '%INSTALL_DIR%\assets\water_level_meter.ico') { $Shortcut.IconLocation = '%INSTALL_DIR%\assets\water_level_meter.ico' } else { $Shortcut.IconLocation = 'C:\Windows\System32\imageres.dll,109' }; $Shortcut.Save() } catch { Write-Host 'Main app shortcut creation failed' }" 2>nul

REM Visualizer shortcut pointing to debug batch file (shows console for visibility)
powershell -ExecutionPolicy Bypass -Command "try { $WshShell = New-Object -comObject WScript.Shell; $Shortcut = $WshShell.CreateShortcut('%INSTALL_DIR%\CAESER Water Level Visualizer.lnk'); $Shortcut.TargetPath = '%INSTALL_DIR%\launchers\water_levels_visualizer_debug.bat'; $Shortcut.WorkingDirectory = '%INSTALL_DIR%'; $Shortcut.Description = 'CAESER Water Level Visualizer'; if (Test-Path '%INSTALL_DIR%\assets\Water_level_tab_icon.ico') { $Shortcut.IconLocation = '%INSTALL_DIR%\assets\Water_level_tab_icon.ico' } else { $Shortcut.IconLocation = 'C:\Windows\System32\imageres.dll,178' }; $Shortcut.Save() } catch { Write-Host 'Visualizer shortcut creation failed' }" 2>nul

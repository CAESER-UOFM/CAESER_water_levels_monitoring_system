...
class PlotHandler(QObject):
            ...
            with sqlite3.connect(db_path) as conn:
                ...
                if date_range:
                    ...
                    if valid_start and valid_end:
                        ...
                        logger.warning("Ignoring invalid date range for baro query")
    ...
    def apply_trend_analysis(self, wells, trend_type="linear", degree=1, style=None):
        ...
        for well in wells:
            ...
            # Get well-specific trend style if available
            if well in self.well_properties and 'trend' in self.well_properties[well]:
                well_style = self.well_properties[well]['trend']
                # Update style with well-specific settings
                for key, value in well_style.items():
                    style[key] = value

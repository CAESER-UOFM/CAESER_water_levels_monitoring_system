    ...
    def get_wells_with_data(self):
        ...
        try:
            with self.get_optimized_connection() as conn:
                ...
                # Build query to only select wells with data - use optimized EXISTS instead of JOIN
                query = f"""
                    SELECT {', '.join(select_columns)}
                    FROM wells w
                    WHERE EXISTS (
                        SELECT 1 FROM water_level_readings r 
                        WHERE r.well_number = w.well_number 
                        LIMIT 1
                    )
                ...
                try:
                    ...
                    logger.error(f"SQL error while executing dynamic query: {e}")
                    # If all else fails, use the most basic query - just get all wells
                    fallback_query = """
                        SELECT well_number, '' as cae_number, '' as water_body
                        FROM wells 
                        ORDER BY well_number
                    """

...
class WaterLevelVisualizer(QDialog):
    ...
    def setup_ui(self):
        ...
        self.resize(1280, 500)  # Reduced height by ~30% from 720 to 500
        ...
        # Main content - now just the upper section - THIS IS THE KEY CHANGE
        upper_section = self.create_upper_section()
        main_layout.addWidget(upper_section, 1)  # Give it a stretch factor to take all space
        
        # Add button to open plot controls
        controls_button = QPushButton("Show Plot Controls")
        controls_button.clicked.connect(self.show_plot_controls)
        main_layout.addWidget(controls_button)
    ...
    def create_plot_panel(self):
        ...
        # Add plot widget and toolbar
        plot_layout.addWidget(self.plot_handler.get_toolbar())
    ...
    def load_wells(self):
        ...
        try:
            ...
            for row, (well_number, cae_number, water_body) in enumerate(wells_data):
                ...
                # Get reading count for this well
                reading_count = self.data_manager.get_reading_count(well_number)
                readings_item = QTableWidgetItem(str(reading_count))
    ...
    def on_well_selection_changed(self):
        ...
        if self.selected_wells:
            ...
            # If we're on the plot tab, clear plot and show loading message immediately
            if self.tab_widget.currentIndex() == 0:  # Plot tab
                self.plot_handler.clear_plot()
                loading_text = self.plot_handler.ax.text(0.5, 0.5, "Loading data...",
                                ha='center', va='center', 
                                transform=self.plot_handler.ax.transAxes,
                                fontsize=14, color='gray',
                                bbox=dict(boxstyle='round,pad=0.5', 
                                    fc='white', ec='gray', alpha=0.8))
                self.plot_handler.canvas.draw()
                self.plot_handler.canvas.flush_events()  # Force update
            ...
            # Load data for each selected well
            self.load_well_data()
    ...
    def update_map(self):
        """Update the map with well locations and selected wells."""
        try:
            # Fetch well locations from data manager
            well_locations = self.data_manager.get_well_locations()
            
            if not well_locations:
                self.status_bar.showMessage("No well locations available for map.")
                return
            
            # Use map_handler to create the map widget
            map_widget = self.map_handler.create_arcgis_map_widget(well_locations, self.selected_wells)
            
            if map_widget:
                # Map widget is already available through map_handler
                self.status_bar.showMessage(f"ArcGIS map updated with {len(well_locations)} wells.")
                return
            
            # Fallback to create_map if the ArcGIS method fails
            map_file = self.map_handler.create_map(well_locations, self.selected_wells)
            
            if map_file:
                # Load map in web view
                self.map_view.load(QUrl.fromLocalFile(map_file))
                self.status_bar.showMessage(f"Map updated with {len(well_locations)} wells.")
            else:
                self.status_bar.showMessage("Failed to create map.")
        ...
        try:
            ...
            for i, well in enumerate(self.selected_wells):
                ...
                if well not in self.plot_handler.well_data or not hasattr(self.plot_handler, 'well_data_full') or well not in self.plot_handler.well_data_full:
                    ...
                    # Get data for this well
                    df = self.plot_handler.get_well_data(well, self.db_path, self.date_range)
    ...
    def apply_trend_analysis(self):
        ...
        try:
            ...
            if trend_type == "Linear":
                self.plot_handler.add_linear_trend(self.selected_wells, style)
                self.status_bar.showMessage(f"Applied linear trend analysis to {len(self.selected_wells)} wells")
            elif trend_type == "Polynomial":
                self.plot_handler.add_polynomial_trend(self.selected_wells, degree=trend_degree, style=style)
                ...
                window = trend_degree * 30  # Convert months to days approximately
                self.plot_handler.add_moving_average(self.selected_wells, window=window, style=style)
